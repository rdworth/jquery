...
XML_LINTER ?= `which xmllint 2>/dev/null`
...
lintdocs:
	@@if test ! -z ${XML_LINTER}; then \
		echo "Linting docs"; \
		for file in ${DOCS_DIR}/*.xml; \
			do ${XML_LINTER} --noout $$file; \
		done; \
	else \
		echo "You must have xmllint installed in order to build docs."; \
	fi

docs: lintdocs

.PHONY: all jquery lint min clean distclean update_submodules pull_submodules pull core docs
